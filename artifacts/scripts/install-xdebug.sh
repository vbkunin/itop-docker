...
echo xdebug.remote_port=${1:-9000} >> ${XDEBUG_CONF}
echo xdebug.remote_host=${REMOTE_HOST} >> ${XDEBUG_CONF}
echo xdebug.idekey=${2:-PHPSTORM} >> ${XDEBUG_CONF}
apache2ctl -k restart

echo ""
echo "You'll probably have to change 'xdebug.remote_host' if you are running on macOS or Windows. Don't forget to restart apache2 in case of any changes: apache2ctl -k restart."
